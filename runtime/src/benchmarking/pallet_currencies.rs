...
runtime_benchmarks! {
	...
	transfer_non_native_currency {
		let amount: Balance = 1_000 * token_unit(STAKING);
	...
	update_balance_non_native_currency {
		let balance: Balance = 2 * token_unit(STAKING);
	...
	//TODO Return verify when existensial deposits are real values
	sweep_dust {
		let c in 1..3u32;
		...
		set_balance(STAKING, &treasury, token_unit(STAKING));
	...
	// 	});
	// 	assert_eq!(Tokens::free_balance(STAKING, &treasury), token_unit(STAKING) + (100 * c) as Balance);
	// }
}
