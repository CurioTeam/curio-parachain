...
use crate::{
	mock::{
		...
		RuntimeEvent as MetaEvent, ExtBuilder, RuntimeOrigin, Session, StakePallet, System, Test, BLOCKS_PER_ROUND, DECIMALS,
		TREASURY_ACC,
	},
	set::OrderedSet,
	types::{
...
fn set_max_selected_candidates_safe_guards() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			assert_noop!(
				StakePallet::set_max_selected_candidates(RuntimeOrigin::root(), <Test as Config>::MinCollators::get() - 1),
				Error::<Test>::CannotSetBelowMin
			);
			assert_noop!(
				StakePallet::set_max_selected_candidates(RuntimeOrigin::root(), <Test as Config>::MaxTopCandidates::get() + 1),
...
fn network_reward_increase_max_candidate_stake() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(StakePallet::set_max_candidate_stake(RuntimeOrigin::root(), max_stake_doubled));
			roll_to(network_reward_start + 2, vec![None]);
